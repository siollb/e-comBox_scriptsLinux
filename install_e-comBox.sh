...
#Gestion de l'adresse IP utilisée pour joindre les sites
echo -e "$COLTXT"
echo -e "Saisissez l'adresse IP que l'utilisateur devra utiliser pour joindre les sites: $COLSAISIE\c"
#echo -e "Si le serveur doit être accessible de l'extérieur, Il faut mettre ici une adresse IP publique" 
read ADRESSE_IP
...
echo "DOCKER_IP_HOST=$ADRESSE_IP" >> /opt/e-comBox_portainer/.env
echo ""
...
do
        sed -i -e "s/localhost:8880/$ADRESSE_IP:8880/g" $fichier
done

echo -e "$COLINFO"
echo "L'application e-comBox est maintenant accessible à l'URL suivante:"
echo -e "http://$ADRESSE_IP:8888"
echo -e ""
